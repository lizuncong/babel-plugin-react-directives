...
export default function SwiperItemCard() {
  ...
  return <Card classNames={[prefixCls, getPrefixCls('swiper-card')].concat(classNames || [])}>
    <span //data-comp_id="link_more"
      ...
      className={cls}></span>
    <button className={clsname([SwiperCard.btn, SwiperCard["btn-large"], SwiperCard.btn__pre])} id='testbtn' />
    <button className={[SwiperCard.listRule1, SwiperCard["btn-sm"], SwiperCard.listRule2, cls].join(" ")} id='okbtn' />
    <span className={SwiperCard.btn}></span>
    <span className={clsname(SwiperCard.btn, SwiperCard.btn_primary, SwiperCard["btn-middel"])}></span>
