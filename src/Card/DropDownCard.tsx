...
export interface IDropDownCard {
  ...
  children?: ReactNode;
  itemSelector: string;
  col?: number; // 正常尺寸下一行的列数
  ratio?: number; // 图片宽高比
  newItemData?: MyGame[];
}
function DropDownCard(prop: IDropDownCard) {
  const { getPrefixCls } = useContext(ConfigContext);
  const prefixCls = getPrefixCls('dropDownCard');
  const { title, step = 4, children, itemSelector, col = 5, ratio = 1 } = prop;
  const { rootWidth, column, className } = useResponsive();
  ...
  return (
    ...
    >
      ...
      <div
        ref={domRef}
        className={classnames([`${prefixCls}__block`, `${show ? '' : 'hide'}`])}
      >
        {children}
      </div>
    </section>
